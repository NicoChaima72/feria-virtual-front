  ...
  const handleSubmit = async (e) => {
    ...
    if (optionSelected === 1) {
      const { meta } = await dispatch(
        acceptAuctionTransportist({ saleId: sale_id })
      );
  ...
  return (
    <DivAnimate>
      <h1 className="text-2xl md:text-3xl font-semibold">
        Resultado subasta de transportistas
      </h1>
      {loadingActive ? (
        ...
        >
          ...
          <hr />
          <div className="bg-gray-50 py-3 px-1">
            {!loadingOptions &&
              options.map((option) => (
...
export default ShowAuctionTransportist;
