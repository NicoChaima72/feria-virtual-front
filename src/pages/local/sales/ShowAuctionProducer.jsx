...
const ShowAuctionProducer = () => {
  const dispatch = useDispatch();
  const { sale_id } = useParams();
  const navigate = useNavigate();
  const { saleActive, loadingActive, error, loadingAuctions, auctions } =
  ...
  const handleSubmit = async (e) => {
    ...
    if (optionSelected === 1) {
      const { meta } = await dispatch(
        acceptAuctionProducer({ saleId: sale_id })
      );
  ...
  return (
    <DivAnimate>
      <h1 className="text-2xl md:text-3xl font-semibold">
        Resultado subasta de productores
      </h1>
      {loadingActive ? (
        ...
        >
          ...
          <p className="pb-1 text-lg">Resultado</p>
          <hr />
          {loadingAuctions ? (
          ...
          <p className="pb-1 text-lg pt-16">Tu información</p>
          <hr />
          <div className="pt-4">
            <label className="block text-sm text-gray-900 dark:text-gray-300">
              RUT
              ...
              {optionSelected == 1 ? (
                <div className="mt-4 mb-6">
                  <p className="text-green-700">
                    IMPORTANTE: Al momento de aceptar el resultado se llevará a
                    cabo la subasta para transporte
                  </p>
                  <button
                    type="submit"
                    className="mt-5 flex items-center justify-center gap-2 text-white font-medium rounded text-sm w-full sm:w-auto px-5 py-2.5 text-center bg-gradient-to-r from-green-700 to-green-500 hover:from-green-600 hover:to-green-400"
                  >
                    Enviar resultado
                  </button>
                </div>
...
export default ShowAuctionProducer;
