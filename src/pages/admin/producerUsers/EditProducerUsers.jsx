...
const EditProducerUsers = () => {
  ...
  const [fruitsVegetablesDefault, setFruitsVegetablesDefault] = useState([]);
  ...
  useEffect(() => {
    if (!loadFruitsVegetables && !loadingActive) {
      const test = fruitsVegetables.filter((f) =>
        userActive.fruits_vegetables.includes(f.label)
      );

      console.log({ test });
    }
  }, [loadFruitsVegetables, loadingActive]);

  useEffect(() => {
    if (!userActive && loadingActive) {
      dispatch(activeUser({ user_id, role_id: 4 }));
    }
          ...
          <div className="mb-6">
            ...
            {!loadFruitsVegetables && !loadingActive ? (
              <Select
                ...
                defaultValue={fruitsVegetables.filter((f) =>
                  userActive.fruits_vegetables.includes(f.label)
                )}
              />
            ) : (
...
export default EditProducerUsers;
